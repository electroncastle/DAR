...
'''
To calculate AP use the  THUMOSevalDet.m scritp from the
official THUMOS evaluation toolbox and run it the following
way:

[pr_all,ap_all,map]=THUMOSevalDet('/home/jiri/Lake/DAR/share/datasets/THUMOS2015/detections.txt','/home/jiri/Lake/DAR/share/datasets/THUMOS2015/TH15_Temporal_annotations_validation/annotations/','validation',0.5);

To print the precision-recall plots
...
class VideoActionsLabeler(object):
    ...
    def process(self, temporal_data, spatial_data, video_filename):
        ...
        classes = (temporal_data[+1:max_frames+1, 1:102]*temp_w + spatial_data[:max_frames, 1:102]*(1.0-temp_w))
        classes_lstm = (temporal_data[:max_frames, 102:203]*temp_w + spatial_data[:max_frames, 102:203]*(1.0-temp_w))
        classes_frame = (temporal_data[:max_frames, 203:304]*temp_w + spatial_data[:max_frames, 203:304]*(1.0-temp_w))
        #classes = classes_lstm
...
#            if (class_val < 0.3):
            ...
            cls = [time, class_id, class_val, self.labels[class_id-1][1] ]
            #print cls
            classes_flt.append(cls)
    ...
    def build_action_list(self, file_name, classes):
        ...
        for i in range(len(classes)):
            ...
            # Create new record
            # [ video_name  start_time  end_time  class  activation  label ]
            current_action = [file_name, cc[0], cc[0], cc[1], cc[2], self.labels[cc[1]-1][1]]
            action_list.append(current_action)
    ...
    def load_detection(self, filename):
        try:
            ...
            #data = np.loadtxt(filename, float, delimiter=' ').astype(dtype=np.float32)
            ...
            print "Cannot read temporal or spatial data. ",filename
            sys.exit(0)
    ...
    def annotate(self, suffix='s5'):
        ...
        fout = self.open_append(dar_root+'/share/datasets/THUMOS2015/detections-fused-'+suffix+'.txt')
        fout_ap = self.open_append(dar_root+'/share/datasets/THUMOS2015/detections-ap-fused-'+suffix+'.txt')
        ...
        for i in range(len(val_list)):
            ...
            # spatial_data = np.loadtxt(video_path+'/spatial.txt', float, delimiter=' ')
            gc.disable()
            temporal_data = np.asarray(self.load_detection(video_path+'/temporal-'+suffix+'.txt'), dtype=np.float32)
            spatial_data = np.asarray(self.load_detection(video_path+'/spatial-'+suffix+'.txt'), dtype=np.float32)
            gc.enable()
